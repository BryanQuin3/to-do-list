 ...
 function agregarTarea(tarea) {
   id++;
   const taskDiv = $("<div>").addClass("flex justify-between items-center mt-5 px-7 ").attr("data-id", id);
   const taskSpan = $("<span>").addClass("text-white text-lg").text(tarea);
   taskDiv.append(taskSpan);
 
   const icons = ["./img/trash.svg", "./img/check.svg"];
   const iconSection = $("<section>").addClass("flex gap-3").append(
     // Add icons to the div
     icons.map((icon) => {
       const iconDiv = $("<div>").addClass("flex justify-center items-center bg-gray-800 rounded-full p-2 hover:bg-gray-700 cursor-pointer");
